...
def get_every_nth_state(states=states, n=10):
    ...
    return [state for index, state in enumerate(states) if (index+1)%n == 0]
...
def get_state_abbrev(state_name, us_state_abbrev=us_state_abbrev):
    ...
    return us_state_abbrev.get(state_name) if state_name in us_state_abbrev.keys() else NOT_FOUND
    # return us_state_abbrev.get(state_name) or NOT_FOUND     # It works too
...
def combine_state_names_and_abbreviations(us_state_abbrev=us_state_abbrev,
    ...
    dict_items = sorted(us_state_abbrev.values()[:10])
    list_items = sorted(states)[-10:]
    return dict_items + list_items
